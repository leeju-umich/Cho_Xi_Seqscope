...
#' @param m_tile sparsecount matrix
#' @export
...
getSimpleGrid = function(seqscope1st,DGEdir,spatial,tiles,nrow,ncol,sidesize,outpath)
{
  ...
  head(df_merge)
  tile_df = df_merge[df_merge$tile_miseq %in% tiles,]
  ...
  obj=apply(tiles,function(x) {collapseTiles(tile_df,x,binx,biny,m_tile)})
